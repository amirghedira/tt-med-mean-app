...
export class AjouterConsultationComponent implements OnInit {
    ...
    onValidate() {
        if (this.agent) {
            if (this.selectedMember !== "default")
                this.doctorService.addConsultation(this.matricule, this.selectedMember, 'other')
                    .subscribe(res => {
                        Swal.fire(
                            'Sucess',
                ...
                this.doctorService.addConsultation(this.agent.matricule.toString(), this.agent._id, 'agent')
                    .subscribe(res => {
                        Swal.fire(
                            'Sucess',
                            'La consultation a été ajoutée',
    ...
    ngOnInit() {
        this.formCtrlSub = this.firstNameControl.valueChanges
            ...
            .subscribe(res => {
                if (this.matricule)
                    this.doctorService.getAgent(this.matricule)
                        .subscribe((res: any) => {
